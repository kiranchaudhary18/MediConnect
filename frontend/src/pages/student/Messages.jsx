...
export default function StudentMessages() {
  ...
  // Load conversations from accepted doctors and assigned patients
  useEffect(() => {
    loadConversations();
  ...
  const loadConversations = async () => {
    try {
      ...
      // Fetch accepted doctors (mentors)
      let doctors = [];
      try {
        const doctorsRes = await axios.get('/patient/student/my-doctors');
        const acceptedDoctors = (doctorsRes.data?.data || [])
          .filter(sel => sel.status === 'accepted')
          .map(sel => ({
            id: sel.doctor?._id || sel.doctor,
            name: sel.doctor?.name || 'Doctor',
            role: 'doctor',
            specialization: sel.doctor?.specialization || 'General',
            avatar: sel.doctor?.profilePicture || null,
      ...
      try {
        ...
        existingChats = (chatsRes || []).map(conv => ({
          id: conv._id || conv.partnerId,
          name: conv.name || conv.partnerName || 'User',
          role: conv.role || 'patient',
          specialization: conv.specialization || '',
          condition: conv.condition || '',
          avatar: conv.profilePicture || conv.avatar || null,
      ...
      // Fetch patients from assignments
      let patients = [];
      try {
        const assignmentsRes = await axios.get('/patient/student/assignments');
        const assignments = assignmentsRes.data?.data || assignmentsRes.data || [];
        
        // Get unique patients from medical records in assignments
        const patientMap = new Map();
        for (const assignment of assignments) {
          if (assignment.medicalRecord?.patient) {
            const patient = assignment.medicalRecord.patient;
            if (!patientMap.has(patient._id)) {
              patientMap.set(patient._id, {
                id: patient._id,
                name: patient.name || 'Patient',
                role: 'patient',
                condition: assignment.medicalRecord?.title || 'Assigned Case',
                avatar: patient.profilePicture || null,
                lastMessage: 'Assigned via case study',
                time: '',
                unread: 0,
                online: false
              });
      ...
      existingChats.forEach(chat => {
        if (!allConversations.find(c => c.id === chat.id)) {
          allConversations.push(chat);
        } else {
          // Update with latest message info
          const idx = allConversations.findIndex(c => c.id === chat.id);
          if (idx !== -1) {
            allConversations[idx].lastMessage = chat.lastMessage;
            allConversations[idx].time = chat.time;
            allConversations[idx].unread = chat.unread;
          }
        }
      });

      // Add patients that aren't already in conversations
      patients.forEach(patient => {
        if (!allConversations.find(c => c.id === patient.id)) {
          allConversations.push(patient);
        }
      });
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="relative">
          ...
          <MessageSquare className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 text-teal-600 animate-pulse" />
        </div>
      </div>
    );
  }
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-teal-500 transition-all"
            />
          </div>
          ...
          <div className="flex gap-2 mt-4">
            {[
              ...
              { id: 'doctor', label: 'Mentors', icon: Stethoscope },
          ...
          {filteredConversations.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">
                {activeTab === 'doctor' 
                  ? 'Select a doctor from Doctors page and wait for acceptance' 
                  : activeTab === 'patient'
                    ? 'Patients will appear when assigned via case studies'
                    : 'Start chatting with your mentors or assigned patients'}
              </p>
            </div>
          ) : (
            filteredConversations.map((conv) => (
              <div
                ...
                className={`p-4 flex items-center gap-3 cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                  selectedConversation?.id === conv.id ? 'bg-teal-50 dark:bg-teal-900/20 border-l-4 border-teal-500' : ''
                }`}
              >
                {/* Avatar */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-teal-600 dark:text-teal-400 mb-0.5">
                    {conv.role === 'doctor' ? (conv.specialization || 'Mentor') : (conv.condition || 'Patient')}
                  </p>
                ...
                {/* Unread Badge */}
                {conv.unread > 0 && (
                  <div className="flex-shrink-0 w-5 h-5 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-full flex items-center justify-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                    ...
                    {selectedConversation.role === 'doctor' 
                      ? (selectedConversation.specialization || 'Mentor') 
                      : (selectedConversation.condition || 'Patient')}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-900/50">
              {messagesLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="w-8 h-8 border-2 border-teal-200 dark:border-teal-800 rounded-full animate-spin border-t-teal-600" />
                </div>
              ) : messages.length === 0 ? (
                <div className="flex items-center justify-center h-full">
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 border-0 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-teal-500 transition-all"
                  />
                </div>
                <button
                  type="submit"
                  disabled={!newMessage.trim() || sending}
                  className="p-3 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-xl hover:shadow-lg hover:shadow-teal-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50 dark:bg-gray-900/50">
            <div className="text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-sm">
                Chat with your accepted mentors (doctors) and assigned patients from case studies.
              </p>
            </div>
          </div>
        )}
